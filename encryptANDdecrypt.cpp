...
string encrypt_worker::getPSWD()
{
    ...
    {
        bool ok;
        QString inputpassword = QInputDialog::getText(nullptr,"输入密码","请输入密码：", QLineEdit::Password, "", &ok);
        password = inputpassword.toStdString();
        inputpassword = QInputDialog::getText(nullptr,"输入密码","请再次输入密码：", QLineEdit::Password, "", &ok);
        password1 = inputpassword.toStdString();
        ...
        {
            QMessageBox::warning(nullptr,"错误","两次输入密码不符，请重新输入!");
        }
    }
    return password;
}
...
bool encrypt_worker::decryptFile(string md5ofpwd,string md5md5ofpwd, string sourcefile,string targetdir)
{
    ...
    {
        QMessageBox::warning(nullptr,"错误","文件格式不符，解密失败！");
        //cout<<"文件格式不符，解密失败！\n";
        return false;
    }    
    int count,i;	
    ...
    {
        QMessageBox::warning(nullptr,"错误","密码错误，退出解密！");
        //cout<<"密码错误，退出解密！\n";
        return false;
    }
    //密码正确则将密码的MD5值转化为密钥
