...
bool compress_worker::compress(string sourcefile, string targetfile)
{
    ...
    {
        QMessageBox::warning(nullptr,"错误","源文件格式不正确");
    ...
    {
        inFile.close();
        QMessageBox::warning(nullptr,"错误","目标文件打开失败");
        return 3; // 打开目标文件失败
    }
...
bool compress_worker::decompress(string sourcePath, string destinationDir)
{
    ...
    if (i != 256)
    {
        QMessageBox::warning(nullptr,"错误","文件过短，频率表不完整");
        // cout << "文件过短\n";
    ...
    while (inFile.read((char *)&uchar, sizeof(unsigned char)))
    {
        ...
        for (char i = 0; i < 8; i++)
        {
            if (now[i] == '0')
            {
                ...
                {
                    QMessageBox::warning(nullptr,"错误","解码错误");
                    // cout << "解码错误\n";
                    return false; // 解码错误
                }
    ...
    {
        QMessageBox::warning(nullptr,"错误","文件结尾不完整");
        // cout << "结尾不完整\n";
