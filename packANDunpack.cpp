...
bool pack_worker::addFileToBag(string dirname, int bagfile)
{
	...
	while ((entry = readdir(dp)) != NULL)
	{
		...
		{
			QMessageBox::warning(nullptr, "警告", "打包过程中遇到不支持的文件类型，已跳过该文件！");
...
bool pack_worker::packDir(string sourcedir, string targetbag)
{
	...
	else
	{
		close(fout);
		// cout << "打包失败!\n";
		QMessageBox::warning(nullptr, "警告", "打包过程中出现错误，打包失败！");
		return false;
	}
}
...
bool pack_worker::turnBagToFile(int sourcebag, string targetdir)
{
	...
	while (read(sourcebag, head, BLOCKSIZE))
	{
		...
		if (head->typeflag == '0')
		{
			...
			{
				QMessageBox::warning(nullptr, "警告", "解包过程中普通文件创建失败！");
				return false;
			}
		...
		else if (head->typeflag == '1')
		{
			...
			{
				QMessageBox::warning(nullptr, "警告", "解包过程中软链接创建失败！");
				// perror("link");
				return false;
			}
		}
		...
		else if (head->typeflag == '2')
		{
			...
			{
				QMessageBox::warning(nullptr, "警告", "解包过程中管道文件创建失败！");
				// perror("pipe");
				return false;
			}
		}
		else
		{
			...
			{
				QMessageBox::warning(nullptr, "警告", "解包过程中目录创建失败！");
				// cout << "创建失败\n";
				return false;
			}
		}
...
bool pack_worker::unpackBag(string sourcebag, string targetdir)
{
	...
	{
		QMessageBox::warning(nullptr, "警告", "当前解包文件格式不符，退出解包！");
	...
	else
	{
		...
		{
			QMessageBox::warning(nullptr, "警告", "解包失败！");
			// cout << "解包失败！\n";
		}
		close(fin);
		return false;
